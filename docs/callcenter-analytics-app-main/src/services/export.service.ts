...
function toExcelCell(value: CellValue, isHeader: boolean) {
  if (value == null || value === '') {
    return null;
  }

  const baseCell = value instanceof Date
    ? { value, type: Date }
    : typeof value === 'number'
      ? { value, type: Number }
  ...
  return isHeader ? { ...baseCell, fontWeight: 'bold' as const } : baseCell;
}

export async function exportToXlsx(exportData: ExportData) {
  const sheets = buildExportSheets(exportData);
  const sheetEntries = Object.entries(sheets);
  const workbookData = sheetEntries.map(([, sheetData]) =>
    sheetData.map((row, rowIndex) =>
      row.map((cell) => toExcelCell(cell as CellValue, rowIndex === 0))
    )
  );

  await writeXlsxFile(workbookData, {
    sheets: sheetEntries.map(([sheetName]) => sheetName),
    fileName: 'reporte_monitoreo_cc.xlsx',
  });
}
...
export function exportToCsv(exportData: ExportData) {
  ...
  link.setAttribute('download', 'reporte_monitoreo_cc.csv');
